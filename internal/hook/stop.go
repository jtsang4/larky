...
func (h StopHandler) continuationPrompt(req *contract.InteractionRequest) string {
	...
	deliveryCommand := fmt.Sprintf("%s delivery record --request-id %s --message-id <MESSAGE_ID> --chat-id %s", shellQuote(binary), shellQuote(req.ID), shellQuote(deliveryChatID))
	failureCommand := fmt.Sprintf("%s delivery fail --request-id %s", shellQuote(binary), shellQuote(req.ID))
	receiptInstruction := "Replace <MESSAGE_ID> with the message_id returned by lark-im."
	if req.ChatID == "" {
		receiptInstruction = "Replace <MESSAGE_ID> and <CHAT_ID_FROM_RESULT> with the message_id and chat_id returned by lark-im."
	}
...
Do not stop until one of those receipt commands succeeds.`,
		target, req.ID, req.Status, req.Project, req.Platform, req.ExpiresAt.Format("2006-01-02T15:04:05Z07:00"), req.ShortCode, req.ShortCode, req.ID, receiptInstruction, deliveryCommand, failureCommand)
}
