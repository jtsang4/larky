...
func (r *Runner) Plan() (Plan, error) {
	...
	return Plan{
		...
		Levels: []LevelPlan{
			...
			{Level: 3, Name: "host-e2e", Purpose: "built binary, real Hook processes, event subprocess, exact-session routing, live rebuild, and host plugin validation", Commands: []string{"make build", "LARKY_BUILT_HOOK_TEST=1 go test ./internal/integration -count=1", "LARKY_ATOMIC_REBUILD_TEST=1 go test ./internal/integration -run TestBuiltBinaryIsAtomicallyReplacedWhileSidecarRuns -count=1", "claude plugin validate plugins/claude --strict"}},
...
func (r *Runner) commands(runDir string, through int) []commandSpec {
	...
	all := []commandSpec{
		...
		{3, "integration", []string{"env", "LARKY_BUILT_HOOK_TEST=1", "go", "test", "./internal/integration", "-count=1"}, false},
