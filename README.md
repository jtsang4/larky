...
在已经配置好 `lark-cli` 和飞书 Skills 的 Mac 上运行：

```bash
curl -fsSL https://raw.githubusercontent.com/jtsang4/larky/main/install.sh | sh
```

脚本会校验 GitHub Release 的 SHA-256，安装 `larky` 到 `~/.local/bin`，并自动为 PATH 中检测到的 Claude Code 和 Codex 安装插件。首次安装后按提示重启 Coding Agent 或新建任务。

以后重复运行同一条命令即可幂等升级，也可以使用 CLI：

```bash
larky update
```

默认升级到最新 Release，并更新检测到的插件。也可以明确选择宿主或版本：
...
如果 `~/.local/bin` 尚未加入 `PATH`，按安装器输出添加即可。只安装命令、不注册插件时，可将 `--binary-only` 传给安装脚本：
...
从源码开发时再运行 `make build`，并把上面的 `larky` 换成 `./dist/larky`。
